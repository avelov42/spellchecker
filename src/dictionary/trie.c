...
#define TRIE_FILE_HEADER L"Lubię rude.."
#define TRIE_FILE_FOOTER L"..ale kocham tylko matmę!"
/* ******************** */
...
int trieDeleteWord(Node* root, const wchar_t* word)
{
    ...
    int ret;
    if((ret = deleteObsoleteNodes(node)) == TRIE_SUCCESS)
    {
 ...
 * @param[in] *filename Nazwa pliku do zapisu o niezerowej długości.
 @todo Sprawdzenie stopy odbywa się po wczytaniu pliku, co jest niebezpieczne przy innych plikach.
 * @return #TRIE_SUCCESS lub #TRIE_ERROR
 * <p>
 * Funkcja dodaje nagłówek i stopkę do pliku w celu możliwości rozpoznania rodzaju pliku i jego
 * kompletności.
 * <p>
 * <strong>Plik o podanej nazwie zostanie nadpisany, a w przypadku gdy nie istnieje- zostanie
 * utworzony. </strong>
 ***********************************************/
int trieSaveToFile(const Node* root, FILE* file)
{
    ...
    ///w tej funkcji, ze wzgledu na specyficzny pomysł na nagłówek biblioteki dictionary
    ///jestem zmuszony założyć, że plik istnieje i jest otwarty w trybie wb..

    //header
    wchar_t* magicBeginPassword = TRIE_FILE_HEADER; //tutaj w sumie moglem nie uzywac tego wskaznika ;)
    if(fwrite(magicBeginPassword, sizeof(magicBeginPassword[0]), wcslen(magicBeginPassword), file)
            != wcslen(magicBeginPassword))
    {
        error();
        return TRIE_ERROR;
    }
    ...
    //zapisanie
    if(saveNodeToFileRec(root, file) != TRIE_SUCCESS)
    {
        error();
        return TRIE_ERROR;
    }
    ...
    //stopa
    wchar_t* magicEndPassword = TRIE_FILE_FOOTER;
    if(fwrite(magicEndPassword, sizeof(magicEndPassword[0]), wcslen(magicEndPassword), file)
            != wcslen(magicEndPassword))
    {
        error();
        return TRIE_ERROR;
    }
...
static int saveNodeToFileRec(const Node* node, FILE* file)
{
    ...
    //jezeli chodz jedna sypnie cos innego niz 1, to reszta sie nie wykona
    //jezeli kazda kolejna bedzie dawac 1, to wykonaja sie wszystkie
    if(fwrite(&(node->value), sizeof(node->value), 1, file) != 1
            || fwrite(&(node->isWord), sizeof(node->isWord), 1, file) != 1
            //|| fwrite(&(node->arraySize), sizeof(node->arraySize), 1, file) != 1
            || fwrite(&(node->childrenCount), sizeof(node->childrenCount), 1, file) != 1)
    {
        error();
        return TRIE_ERROR;
    }
    for(int i = 0; i < node->childrenCount; i++)
    {
        ...
        if(saveNodeToFileRec(node->childrenArray[i], file) == -1)
        {
            error();
            return TRIE_ERROR;
        } //tutaj juz sie zahamuje z tymi komunikatami ;)
    }
    return TRIE_SUCCESS;
}
...
*
* @param[in] *file Wskaźnik do otwartego pliku w trybie wb.
* @return Wskaźnik na root-a wczytanego drzewa. W przypadku niepowodzenia - NULL.
*
* Sprawdza, czy nagłówek oraz stopa pliku są prawidłowe.
*
***********************************************/
Node* trieLoadFromFile(FILE* file)
{
    ...
    ///tutaj rowniez zakladamy, ze plik jest poprawnie otwarty, por. trieSaveToFile.
    ///AAARGH!

    //naglowek
    int magicBeginPasswordLength = wcslen(TRIE_FILE_HEADER);
    wchar_t beginPassword[magicBeginPasswordLength+1];
    beginPassword[magicBeginPasswordLength] = L'\0'; //konieczne do porownania stringow

    if(fread(beginPassword, sizeof(beginPassword[0]), magicBeginPasswordLength, file)
            != wcslen(beginPassword))
    {
        error(); //blad przy wczytywaniu znakow header'a
        return NULL;
    }

    else if(wcscmp(TRIE_FILE_HEADER, beginPassword) != 0) //zly format pliku
    {
        fprintf(stderr, "File is incorrect format.\n");
        return NULL; //tutaj pozwole sobie recznie obsluzyc problem
    }
    ...
    //wczytanie stopki
    int magicEndPasswordLength = wcslen(TRIE_FILE_FOOTER);
    wchar_t endPassword[magicEndPasswordLength+1];
    endPassword[magicEndPasswordLength] = L'\0'; //konieczne do porownania stringow

    if(fread(endPassword, sizeof(endPassword[0]), magicEndPasswordLength, file)
            != wcslen(endPassword))
    {
        error();
        return NULL; //patrz wczytywanie naglowka
    }

    else if(wcscmp(TRIE_FILE_FOOTER, endPassword) != 0)
    {
        fprintf(stderr, "File is broken.");
        assert(returnValue != NULL);
        trieDeleteTrie(returnValue);
        return NULL;
    }

    ///uwaga: funkcja pozostawia plik otwarty
    return returnValue;
}
...
static Node* loadNodeFromFileRec(Node* parent, FILE* file)
{
    ...
    if(fread(&(node->value), sizeof(node->value), 1, file) != 1
            || fread(&(node->isWord), sizeof(node->isWord), 1, file) != 1
            //|| fread(&(node->arraySize), sizeof(node->arraySize), 1, file) != 1
            || fread(&(node->childrenCount), sizeof(node->childrenCount), 1, file) != 1)
    {
        error();
        return NULL;
    }
    ...
    for(int i = 0; i < node->childrenCount; i++)
    {
        if((node->childrenArray[i] = loadNodeFromFileRec(node, file)) == NULL)
        {
            error();
            return NULL;
        }
